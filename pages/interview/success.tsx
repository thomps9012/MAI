...
export default function Success() {
    ...
    const { data: card_data, error: card_err } = useSWR(`/api/cards/find?interview_id=${interview_data.id}`, fetcher)
    if (card_err) return <main className="landing"><h1>Trouble Connecting to the Database... <br /> Check Your Internet or Cellular Connection</h1></main>
    ...
    return <main className="landing">
        ...
        {user_data.admin && <>
            ...
            <a className='landing-link' onClick={() => sessionStorage.clear()}><Link href='/admin/clients'>Review All Clients</Link></a>
            <a className='landing-link' onClick={() => sessionStorage.clear()}><Link href={`/interview_detail/${interview_data.id}`}><>Review Client {interview_data.PID} {titleCase(interview_data.type.split("_").join(" "))} Interview</></Link></a>
            <a className='landing-link' onClick={() => sessionStorage.clear()}><Link href='/admin/interviews'>Review All Interviews</Link></a>
        </>}
    </main>
