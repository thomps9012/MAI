...
export default function CardRecordsPage({ card_records }: any) {
    ...
    const filter = () => {
        ...
        const dispersed_filter = (document.getElementById('dispersed') as HTMLInputElement)?.value
        if (dispersed_filter === "") {
            selected_agency === "" ?
            ...
            selected_agency === "" ?
            setGiftCards(card_records.filter((record: any) => (JSON.parse(dispersed_filter)) ? record.received_date : !record.received_date).filter((record: any) => record.PID.includes(PID_input)))
            :
            setGiftCards(card_records.filter((record: any) => (JSON.parse(dispersed_filter)) ? record.received_date : !record.received_date).filter((record: any) => record.PID.includes(selected_agency) && record.PID.split(selected_agency)[1].includes(PID_input)))
        }
    }
