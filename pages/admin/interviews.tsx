...
export default function InterviewRecordsPage({ baseline_records, testing_only_records, follow_up_records, exit_records }: any) {
    ...
    const filter_by_agency = (agency: string) => {
        setBaselines(baseline_records.filter((record: any) => record.agency === agency))
        setTestingOnly(testing_only_records.filter((record: any) => record.agency === agency))
        setFollowUps(follow_up_records.filter((record: any) => record.agency === agency))
    ...
    const filter_by_PID = (PID: string) => {
        ...
        setTestingOnly(testing_only.filter((record: any) => record.PID.includes(PID)))
    ...
    return <main className="container">
        ...
        <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'flex-start', flexWrap: 'wrap' }}>
            <div style={{ flexDirection: 'column' }}>
                <h3>Agency</h3>
                <select name='agency' id='agency' onChange={filter_records} defaultValue="">
            ...
            <div style={{ flexDirection: 'column' }}>
                <h3>Type</h3>
                <select multiple name='interview_type' id='interview_type' onChange={filter_records} defaultValue="">
                    <option value="baseline" className="interview_type">Baseline</option>
                    <option value="testing_only" className="interview_type">Testing Only</option>
                    <option value="follow_up" className="interview_type">Follow Up</option>
                    <option value="exit" className="interview_type">Exit</option>
                </select>
            </div>
        </div>
        {baselines?.length > 0 && <section className="interview_overviews">
            <h1 id="baseline">Baseline Interviews</h1>
            <hr />
            {baselines.map((record: any) => <InterviewOverview key={record._id} record={record} type={'baseline'} />)}
        </section>}
        {testing_only?.length > 0 && <section className="interview_overviews">
            <h1 id="testing_only">Testing Only Interviews</h1>
            <hr />
            {testing_only.map((record: any) => <InterviewOverview key={record._id} record={record} type={'testing_only'} />)}
        </section>}
        {follow_ups?.length > 0 && <section className="interview_overviews">
            <h1 id="follow_up">Follow Up Interviews</h1>
            <hr />
            {follow_ups.map((record: any) => <InterviewOverview key={record._id} record={record} type={'follow_up'} />)}
        </section>}
        {exits?.length > 0 && <section className="interview_overviews">
            <h1 id="exit">Exit Interviews</h1>
            <hr />
            {exits.map((record: any) => <InterviewOverview key={record._id} record={record} type={'exit'} />)}
        </section>}
    </main>
}
