...
export default function NavBar() {
    ...
    return <nav>
        <ul className="web-nav">
            ...
            {!user_info.logged_in && <li className="nav-link" id="/sign_in"><Link href="/sign_in"><a>Login</a></Link></li>}
            {!user_info.logged_in && <li className="nav-link" id="/sign_up"><Link href="/sign_up"><a>Sign Up</a></Link></li>}
            ...
            {user_info.logged_in && <li className="nav-link" id="/admin/users/[id]"><Link href={`/admin/users/${user_info.user?.id}`}><a>Profile</a></Link></li>}
        </ul>
        <div className="mobile-menu">
            <a onClick={() => setShow(!show)}><p>Menu</p></a>
            {user_info.logged_in && <a onClick={() => router.push(`/admin/users/${user_info.user?.id}`)}><p>{user_info.user?.full_name}</p></a>}
            {user_info.logged_in && <a onClick={logout}><p>Logout</p></a>}
        </div>
        <ul className={`mobile-nav-${show ? 'show' : 'hide'}`} onMouseLeave={() => setShow(false)}>
            ...
            {!user_info.logged_in && <li className="nav-link" onClick={() => setShow(false)} id="/sign_in"><Link href="/sign_in"><a>Login</a></Link></li>}
            {!user_info.logged_in && <li className="nav-link" onClick={() => setShow(false)} id="/sign_up"><Link href="/sign_up"><a>Sign Up</a></Link></li>}
            {user_info.user?.admin && <>
